...
@pytest.mark.parametrize("snp,r_thresh,done, expected", [
    ...
    ('SNP1', 0.5, ['SNP4', 'SNP7'], ['SNP1', 'SNP8', 'SNP12', 'SNP13', 'SNP14',
                                     'SNP15']),
    ('SNP5', 0.5, [], ['SNP5', 'SNP11']),
    ('SNP5', 0.2, ['SNP1', 'SNP4', 'SNP7', 'SNP8'], ['SNP5', 'SNP11', 'SNP12',
                                                     'SNP13', 'SNP14', 'SNP15'])
])
...
@pytest.mark.parametrize("r_thresh,p_thresh,expected", [
    (0.5, 1E-16, {'SNP1': ['SNP1', 'SNP4', 'SNP7', 'SNP8', 'SNP12', 'SNP13',
    'SNP14', 'SNP15'], 'SNP10': ['SNP10'], 'SNP3': ['SNP3'], 'SNP5': ['SNP5',
    'SNP11']}),
    ])
def test_clump(r_thresh, p_thresh, expected):
    clumps = clump(R2, ss, r_thresh, p_thresh)
    assert clumps == expected
...
def test_score(r_t, p_t,expected):
    r, p, r2, prs = score(X, bim, y, ss, r_t, p_t, R2)
    assert (r, p) == (r_t, p_t)
...
def test_pplust(r_range, p_range, expected):
    p_t, r_t, r2, clumps, prs = pplust('testing', X, y, bim, ss, r_range, p_range)
