...
python3 ${code}/qtraitsimulation.py -p AFR -m 100 -b 0.8 -f 0 -B ${main_target} -2 ${main_source} -t ${cpus} --causal_eff EUR.causaleff --normalize
cat EUR.pheno AFR.pheno > train.pheno

step=$(( sample/10 ))
for i in `seq 0 $step $sample`
do 
    if [[ ! $i = 0 ]]; then head -n $i ${genos}/AFR.train > ${i}.keep;fi
    eur=$(( sample - i ))
    if [[ ! $eur = 0  ]]; then head -n $eur ${genos}/EUR.train >> ${i}.keep;fi
    $plink --bfile ${genos}/EURnAFR --pheno train.pheno --keep ${i}.keep --keep-allele-order --allow-no-sex --make-bed --out ${i} --threads ${cpus} --memory $(( mem/1000000 ))
