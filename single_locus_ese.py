...
def random_locus_yielder(rbfile, tbfile, window, f_thr, threads, flip,
    ...
    tbim = tbim[rbim.i.isin(tindices)].reset_index(drop=True)
    ...
    while True:
        ...
        rg, tg = rg[:, ridx], tg[:, tidx]
        yield rg, rfam, tg, tfam, df
...
def compute_individual(tgeno, tpheno, loci, intfile, sumstats, avh2, h2, n,
    ...
    if not os.path.isfile(intfile):
        ...
        df = prune_it(sorted_df, tgeno, tpheno, 'Integral', step=1, n=1,
...
def compute_all(prefix, index, locus, refl, tarl, seed, maxmem, avoid_causals,
    ...
    # compute phenotypes
    rbim = df.reindex(columns=['chrom', 'snp', 'cm', 'pos', 'a0', 'a1', 'i_ref',
                               'mafs', 'flip']).rename(columns={'i_ref':'i'})
    tbim = df.reindex(columns=['chrom', 'snp', 'cm', 'pos', 'a0', 'a1', 'i_tar',
                               'mafs', 'flip']).rename(columns={'i_ref': 'i'})
    opts = dict(outprefix=refl, bfile=rg, h2=0.05, ncausal=1, normalize=True,
                uniform=False, snps=None, seed=seed, bfile2=tg, flip=False,
                max_memory=maxmem, freq_thresh=0, remove_causals=avoid_causals,
                bim=rbim, fam=rfam)
    rpheno, h2, (rgeno, rbim, rtruebeta, rcausals) = qtraits_simulation(**opts)
    # make simulation for target
    opts.update(dict(outprefix=tarl, bfile=tg,  bfile2=rg, causaleff=rcausals,
                     fam=tfam, bim=tbim))
    tpheno, h2, (tgeno, tbim, truebeta, tcausals) = qtraits_simulation(**opts)
    # Compute summary statistics
    name = 'Locus%s' % str(index)
    ...
    intfile = '%s_%s_res.tsv' % (prefix, 'integral')
    integral_df, integral = compute_individual(tgeno, tpheno, loci, intfile,
                                               sumstats, avh2, h2, n, scs_title,
                                               True, 'ese', maxmem, threads)
    # include ppt
    pptfile = '%s_%s_res.tsv' % (prefix, 'ppt')
    n, m = tgeno.shape
    if not os.path.isfile(pptfile):
        opts = dict(by_range=None, sort_by='pvalue', loci=loci, h2=h2, m=m, n=n,
                    ...
                    tgeno=tgeno, prefix='%s_pval_all' % prefix,
                    select_index_by='pvalue', clump_with='d_reference',
                    do_locus_ese=False, normalize=True)
        # run standard P + T
        ppt = run_optimization_by(**opts)
        snps = ppt['index_snps'][0]
        ppt = prune_it(sumstats[sumstats.snp == snps], tgeno, tpheno, 'P + T',
    ...
    eses = [
        individual_ese(sumstats, avh2, h2, n, x, loci, tgeno, tpheno, threads,
                       tbim, prefix, maxmem, prune_n=1) for x in [0, 1, 2]]

    # prune by pval
    resfile = '%s_%s_res.tsv' % (prefix, 'pvalue')
...
def main(args):
    ...
    for i in range(args.runs):
        locus = random_locus_yielder(args.refgeno, args.targeno, args.window,
                                     0.01, args.threads, False, False, memory)
        result = compute_all(args.prefix, i, locus, refl, tarl, seed, memory,
                             args.avoid_causals, args.threads, args.window)
        result['run'] = i
        res.append(result)
