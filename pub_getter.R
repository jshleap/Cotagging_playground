...
# crossref GWAS 1013 (total) 981 (journals?)
if (file.exists("CROSSREF.R")){
  load("CROSSREF.R")} else {
  crossrefgwas <- ft_search(query = "GWAS", from = "crossref",
  crossrefopts=list(filter=list(type="journal-article")), limit=981)
  # crossref UKBB 1
  crossrefukbb <- ft_search(query = "UKBB", from = "crossref",
  filter=c(type="journal-article", has_full_text = TRUE))
  # crossref UK BioBank 68232
  crossrefukbb2 <- rcrossref::cr_works(query="'UK biobank'", cursor='*',
  cursor_max=68232, filter=c(type="journal-article", has_full_text = TRUE))
...
if (file.exists("ENTREZ.R")){
  load("ENTREZ.R")} else {
  entrez <- ft_search(query = 'GWAS AND "UK biobank" AND homo[organism]', from = "entrez")
  ENTREZ <- entrez$data$doi
...
if (file.exists("ARXIV.R")){
  ...
  # arxiv UKBB 1
  arxivukbb <- ft_search(query = 'UKBB', from = "arxiv", limit=1)
  # arxiv UK BioBabnk 17
  arxivukbb1 <- ft_search(query = '"UK biobank"', from = "arxiv", limit=17)
  ARXIV <- intersect(unique(c(arxivukbb$arxiv$data$doi, arxivukbb2$arxiv$data$doi)), arxivgwas$arxiv$data$doi)
